...
class _CustomAppBarState extends State<CustomAppBar> {
  ...
  Future<void> _updateStreak() async {
    ...
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      if (lastLoginDate == today.subtract(const Duration(days: 1))) {
        currentStreak += 1; // Consecutive login, increase streak
      } else if (lastLoginDate.isBefore(today.subtract(const Duration(days: 1)))) {
  ...
  Widget build(BuildContext context) {
    return AppBar(
      ...
      backgroundColor: const Color.fromARGB(255, 26, 130, 6),
      title: Row(
